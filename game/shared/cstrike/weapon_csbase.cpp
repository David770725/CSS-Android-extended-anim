...
SendPropInt(SENDINFO(m_nHolsterStage), 3, SPROP_UNSIGNED),
#else
RecvPropInt(RECVINFO(m_nInspectStage)),
RecvPropInt(RECVINFO(m_nHolsterStage)),
...
int CWeaponCSBase::GetHolsterActivity(void)
{
;
}
int CWeaponCSBase::GetInspectActivity(void)
{
;
}

void CWeaponCSBase::HandleInspect(void)
{
	CCSPlayer *pPlayer = GetPlayerOwner();
	if (!pPlayer)
		return;

if(pPlayer->GetFOV() == 80) //bill: disable when sight
return;

int Anim;
if(m_iClip1 == 0)
Anim = ACT_PRIMARY_VM_INSPECT_END;
else if(m_iClip1 > 0)
Anim = ACT_PRIMARY_VM_INSPECT_START;

SendWeaponAnim(Anim);

if ( gpGlobals->curtime >= SequenceDuration( Anim ) )
...
bool firstDraw;
bool isHolster;
void CWeaponCSBase::HandleHolster(void)
{
	CCSPlayer *pPlayer = GetPlayerOwner();
	if (!pPlayer)
		return;
...
ProcessAnimationEvents();//sprint

if (CanInspect())
{
	if (pPlayer->m_afButtonPressed & IN_INSPECT)
		HandleInspect();

	if (pPlayer->m_afButtonReleased & IN_INSPECT)
		m_nInspectStage = 2;
}
...
if (CanHolster())
{
	...
	if (pPlayer->m_afButtonPressed & IN_HOLSTER)
		HandleHolster();

	if (pPlayer->m_afButtonReleased & IN_HOLSTER)
		m_nHolsterStage = 2;
...
void CWeaponCSBase::ProcessAnimationEvents(void)
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	if (!pOwner)
		return;

if(pOwner->GetFOV() == 80 || isHolster) //bill: disable when sight
return;
...
else
{
	...
	if ( !m_bWeaponIsLowered && (pOwner->m_nButtons & IN_FORWARD || pOwner->m_nButtons & IN_BACK || pOwner->m_nButtons & IN_MOVELEFT || pOwner->m_nButtons & IN_MOVERIGHT) )
	{
...
m_bWeaponIsLowered = true;
	...
	else if ( m_bWeaponIsLowered && !(pOwner->m_nButtons & IN_FORWARD || pOwner->m_nButtons & IN_BACK || pOwner->m_nButtons & IN_MOVELEFT || pOwner->m_nButtons & IN_MOVERIGHT ) )
	{
...
Activity CWeaponCSBase::DRAW( void )
{

	return ACT_VM_DRAW;
}

Activity CWeaponCSBase::DRAW_EMPTY( void )
{
...
bool CWeaponCSBase::DefaultDeployFirst( char *szViewModel, char *szWeaponModel, int iActivity, char *szAnimExt )
{
...
isHolster = FALSE;
	...
	SendWeaponAnim( DRAW_EMPTY() );

	//pOwner->SetNextAttack( gpGlobals->curtime + 1 );
	//m_flNextPrimaryAttack	= gpGlobals->curtime;
	//m_flNextSecondaryAttack	= gpGlobals->curtime;
...
bool CWeaponCSBase::Deploy()
{
	...
	return DefaultDeploy( (char*)GetViewModel(), (char*)GetWorldModel(), DRAW_EMPTY(), (char*)GetAnimPrefix() );//BaseClass::Deploy();
}
...
if(pPlayer->GetFOV() <= 85) //bill: hide when sight
return;
...
SendWeaponAnim( DRAW() );
//DefaultDeployFirst( (char*)GetViewModel(), (char*)GetWorldModel(), DRAW(), (char*)GetAnimPrefix() );//BaseClass::Deploy();
    }
...
firstDraw = TRUE;
SetWeaponVisible( true );
pPlayer->GetActiveCSWeapon()->DefaultDeployFirst( (char*)GetViewModel(), (char*)GetWorldModel(), DRAW(), (char*)GetAnimPrefix() );
